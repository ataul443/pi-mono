...
export function parseArgs(args: string[]): Args {
	...
	for (let i = 0; i < args.length; i++) {
		...
		} else if (arg === "--worktree" && i + 1 < args.length && !args[i + 1].startsWith("-")) {
...
${chalk.bold("Options:")}
  ...
  --tools <tools>                Comma-separated list of tools to enable (default: Read,Bash,Edit,Write)
                                 Available: Read, Bash, Edit, Write, Grep, Glob, List, web_search, web_fetch
  ...
  --worktree <slug>               Create and run in a git worktree with the given slug
...
${chalk.bold("Examples:")}
  ...
  ${APP_NAME} --tools Read,Grep,Glob,List -p "Review the code in src/"
...
${chalk.bold("Available Tools (default: Read, Bash, Edit, Write):")}
  ...
  Glob   - Find files by glob pattern (read-only, off by default)
  List   - List directory contents (read-only, off by default)
`);
}
