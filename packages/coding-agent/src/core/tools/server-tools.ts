 ...
 */
export const SERVER_TOOL_NAMES = new Set(["WebSearch", "WebFetch"]);
...
export const serverToolDefinitions: ToolDefinition[] = [
	{
		...
		async execute() {
			throw new Error("WebSearch is an Anthropic server-side tool and cannot be executed locally");
		},
	},
	{
		name: "WebFetch",
		...
		parameters: Type.Object({}),
		async execute() {
			throw new Error("WebFetch is an Anthropic server-side tool and cannot be executed locally");
		},
	},
];
